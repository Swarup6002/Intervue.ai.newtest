...
def start_interview(req: StartRequest):
    ...
    success, error_msg = update_session(session_id, "Easy", initial_history, user_id=req.user_id)
    if not success:
...
def submit_answer(req: AnswerRequest):
    ...
    success, error = update_session(req.session_id, new_diff, history, user_id=None)
    if not success:
        print(f"⚠️ Failed to save progress: {error}")
